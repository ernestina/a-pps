...
class DataUser {
    ...
    public function get_d_user($limit = null, $batas = null) {
        $sql = "SELECT * FROM " . $this->_table . " ORDER BY kd_user";
        ...
        foreach ($result as $val) {
            ...
            $d_user->set_kd_user($val['kd_user']);
    ...
    public function get_d_user_by_id($d_user = DataUser) {
        if (is_null($d_user->get_kd_user())) {
            return false;
        }
        $sql = "SELECT * FROM " . $this->_table . " WHERE kd_user=" . $d_user->get_kd_user();
//        var_dump($sql);
        $result = $this->db->select($sql);
        foreach ($result as $val) {
            $this->set_kd_user($val['kd_user']);
    ...
    public function get_kanwil_name() {
        $sql = "SELECT nama_user FROM d_user WHERE kd_r_jenis=5";
    ...
    public function update_d_user() {
        ...
        $where = ' kd_user=' . $this->get_kd_user();
    ...
    public function set_kd_user($kode) {
        $this->_kd_user = $kode;
    }
    ...
    public function get_kd_user($where = null) {
        if (!is_null($where)) {
            $sql = "SELECT kd_user FROM '" . $this->_table . "' WHERE '" . $where . "'";
            $result = $this->db->select($sql);
            foreach ($result as $val) {
                $this->set_kd_user($val['kd_user']);
            }
